...
def plot_best_type_layouts(agent_types, questionnaire_file_name, layouts_prefix, trained_on_number_of_layouts):
    ...
    best_agent_type_layouts = df_max_rewards[df_max_rewards == df.groupby('Which layout? (write the layout name as it exactly appears on the repository)')['Reward'].max()].index
    ...
    plt.savefig(f'{QUESTIONNAIRES_DIR}/{questionnaire_file_name}/best_{agent_types}_in_{layouts_prefix}_layouts_trained_on_{trained_on_number_of_layouts}_layouts.png', dpi=100)


if __name__ == "__main__":
    questionnaire_file_name = '2'
    layouts_prefix = ''
    agent_types = ['N-1-SP with curriculum', 'N-1-SP without curriculum']
