...
df_3_chefs = df[df['Which layout? (write the layout name as it exactly appears on the repository)'].str.contains('5_chefs', na=False)]
...
unique_layouts = df_3_chefs['Which layout? (write the layout name as it exactly appears on the repository)'].unique()
...
for i, layout in enumerate(unique_layouts):
    layout_df = df_3_chefs[df_3_chefs['Which layout? (write the layout name as it exactly appears on the repository)'] == layout]
...
plt.tight_layout()
plt.savefig('data/questionnaire/5_chefs_layouts.png', dpi=100)
