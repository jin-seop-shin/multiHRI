...
# Group by layout and find the highest reward for Self play agents
sp_max_rewards = df_3_chefs[df_3_chefs['Agent Type'] == 'Self play'].groupby('Which layout? (write the layout name as it exactly appears on the repository)')['Reward'].max()

# Filter layouts where Self play agents have the highest reward
best_sp_layouts = sp_max_rewards[sp_max_rewards == df_3_chefs.groupby('Which layout? (write the layout name as it exactly appears on the repository)')['Reward'].max()].index

# Filter the dataframe to include only the best Self play layouts
df_best_sp = df_3_chefs[df_3_chefs['Which layout? (write the layout name as it exactly appears on the repository)'].isin(best_sp_layouts)]

unique_layouts = best_sp_layouts.unique()
...
for i, layout in enumerate(unique_layouts):
    layout_df = df_best_sp[df_best_sp['Which layout? (write the layout name as it exactly appears on the repository)'] == layout]
    ...
    axes[i, 0].set_title(f"Layout: {layout} - Sorted by Reward (Self play is best)")
...
plt.tight_layout()
plt.savefig('data/questionnaire/best_sp_layouts.png', dpi=100)